...
export const ExamListPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 font-sans text-left">
                ...
                <div className="space-y-3 flex-1">
                  <div>
                    <span className="px-2.5 py-0.5 bg-slate-100 text-slate-600 rounded text-[9.5px] font-black uppercase tracking-wider">
                      {exam.courseId === 'ALL' ? 'All Modules' : (course?.courseName || 'Module Exam')}
                    </span>
      ...
      {showModulesModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-xl border border-slate-100 flex flex-col gap-4 text-left">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <h3 className="text-base font-extrabold text-slate-800 tracking-tight">Manage Exam Modules</h3>
              <button 
                type="button"
            ...
            <form onSubmit={async (e) => {
              ...
              try {
                ...
                await api.post('/api/v1/exam-courses', { courseName: newModuleName.trim() });
                setNewModuleName('');
                toast.success('Exam module added successfully!');
                // Reload list
                const data = await api.get<any[]>('/api/v1/exam-courses');
                setCourses(data);
              } catch (err: any) {
                console.error(err);
              ...
              <input
                ...
                placeholder="e.g. PRF, OOP, Internet technologies"
                className="flex-1 px-4 py-2 border border-slate-200 focus:border-[#4F3FF0]/60 rounded-xl outline-none text-xs font-semibold text-slate-800 bg-slate-50"
              />
              <button
                type="submit"
                disabled={savingModule}
                className="px-4 py-2 bg-[#4F3FF0] hover:bg-[#3D2ED0] text-white text-xs font-black rounded-xl transition-all cursor-pointer shadow-sm shadow-[#4F3FF0]/10 flex items-center gap-1.5"
              >
                Add
              </button>
            </form>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Active Exam Modules</span>
              {courses.length === 0 ? (
                <div className="text-center py-6 text-xs text-slate-400 font-medium">No exam modules registered yet.</div>
              ) : (
                courses.map(c => (
                  <div key={c.courseId} className="flex justify-between items-center bg-slate-50 px-4 py-2.5 rounded-xl border border-slate-100">
                    <span className="text-xs font-bold text-slate-850">{c.courseName}</span>
                    <button
                      type="button"
                      onClick={async () => {
                        if (!window.confirm(`Are you sure you want to delete "${c.courseName}"?`)) return;
                        try {
                          await api.delete(`/api/v1/exam-courses/${c.courseId}`);
                          ...
                          console.error(err);
                          toast.error('Failed to delete exam module.');
                        }
                      }}
                      className="p-1 hover:bg-slate-200 text-rose-500 hover:text-rose-700 rounded-lg transition-colors cursor-pointer"
                    >
